...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defvar *loop-keywords*
    '(:for :do :repeat :with :when :unless :while :until :finally
      :from :to :below :downto :above :by :in :across :on := :then :of
...
(defun a-with-clause (state) ;; so named to avoid with-xxx macro convention
  (let ((place (eat state)))
    (eat state :=)
    (push (list place (eat state)) (prologue state))))
...
(defpsmacro loop (&rest keywords-and-forms)
  (let* ((state (parse-ps-loop keywords-and-forms))
    ...
    (if (implicit-accum-var state)
        `((lambda () ,@full)) ;; generalize this to support cl-style RETURN
        `(progn ,@full))))
